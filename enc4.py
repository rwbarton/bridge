...
def main():
    ...
    handsE = np.zeros((ROWS, 52), dtype=np.float32)
    handsS = np.zeros((ROWS, 52), dtype=np.float32)
    handsW = np.zeros((ROWS, 52), dtype=np.float32)
    bids = [np.full(ROWS, -1, dtype=np.int8) for _ in range(max_auction_length)]
    bar = progressbar.ProgressBar()

    f = open(input_file, 'r')
    for i in bar(range(ROWS)):
        l = f.readline()
        (handN, handE, handS, handW), auction = (eval(col) for col in l.rstrip().split('\t'))
        handsN[i] = np.array(hand_to_vec(handN), dtype=np.float32)
        handsE[i] = np.array(hand_to_vec(handE), dtype=np.float32)
    ...
    np.save(os.path.join(OUTPUT_DIR_HANDS, 'handsE'), handsE)
    np.save(os.path.join(OUTPUT_DIR_HANDS, 'handsS'), handsS)
    np.save(os.path.join(OUTPUT_DIR_HANDS, 'handsW'), handsW)
